...
# configuration
file.path = '/Users/woodie/Desktop/workspace/Avoiding-Bias-Data/data/stole.biased.keywords.txt'
...
colnames(comparison.df) = c('Stole.TFIDF', 'Keywords.TFIDF', 'keywords')
for (i in 3:ncol(keywords.df)) {
  ...
  colnames(sub.comparison.df) = c('Stole.TFIDF', 'Keywords.TFIDF', 'keywords')
  comparison.df = rbind(comparison.df, sub.comparison.df)
}
comparison.df = comparison.df[
  comparison.df$Stole.TFIDF>0 & 
  comparison.df$Stole.TFIDF<0.25 & 
...
p <- ggplot(comparison.df, aes(Keywords.TFIDF, Stole.TFIDF)) +
